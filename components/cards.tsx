    ...
    >
      ...
      <div className="p-6 md:p-8 flex-1 flex flex-col">
        ...
        <h3 className={cn(
          ...
          featured ? 'text-2xl md:text-3xl leading-tight' : 'text-xl leading-snug'
        )}>
          {title}
        </h3>
...
export function SectorCard({ title, description }: SectorCardProps) {
  return (
    <div className="p-6 md:p-8 rounded-lg bg-card border border-border hover:border-primary/40 hover:shadow-md transition-all group cursor-pointer">
...
export function TeamMemberCard({ name, title, expertise, image }: TeamMemberCardProps) {
  return (
    <div className="publication-card flex flex-col">
      ...
      <div className="p-6 md:p-8 flex-1 flex flex-col">
        <h3 className="font-serif font-bold text-lg text-foreground mb-1">{name}</h3>
        <p className="text-sm text-primary font-semibold mb-4">{title}</p>
        <div className="flex flex-wrap gap-2">
          {expertise.map((exp) => (
            <span key={exp} className="text-xs px-3 py-1.5 bg-muted text-muted-foreground rounded-full font-medium">
              {exp}
            </span>
          ))}
        </div>
